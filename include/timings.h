...
#define DRV_TYPE_SHOW_TIME      (5.000 * PRD3)      // Время индикации типа привода при пуске блока
#define DRV_TEMPER_TOUT			(1.000 * PRD2)		// Время задержки срабатывания защиты по перегреву двигателя
...
#define DEBUG_START_TIME		(2.000 * PRD1)      // таймер для повторного включения ПП без источника

#define NET_MON_START_TIME		(1.000 * PRD2)		// Задержка перед пуском по наличию напряжения
#define NET_MON_STOP_TIME2 		(0.500 * PRD2)		// Задержка перед перед стопом при пропаже напряжения
...
#define FLT_DEF_DELAY			(4.000 * PRD4)	// Задержка на определение всех аварий
