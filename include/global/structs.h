...
typedef union _TOutputReg {
	...
	struct {
		...
		Uns Dout7:1;		// 7 - Му/Ду/ ---
		Uns Dout8:1;		// 8 - МВЗ/неисправность
		Uns Dout9:1;		// 9   Тип входного сигнала/ КВЗ
		Uns Dout10:1;		// 10  --/ КВО	
...
typedef union _TTEK_DefReg
{
	...
	struct 
	{
		...
		Uns Rsvd4:1;		// 4 	Обрыв фазы электродвигателя
...
#else
		Uns Rsvd12:1;		// 12   Неверное чередование фаз сети
#endif
		Uns PhOrdDrv:1;		// 13   Чередование фаз двигателя
		Uns Rsvd14:1;		// 14 	Дефект устройства (кроме перегрева и переохлаждения)
		Uns Rsvd15:1;		// 15 	Резерв
	} bit;
} TTEK_DefReg;
...
typedef struct
{
	...
	Uns		onTimeout;		// Параметр: Время удержания флага
