...
void ProtectionI2T_Update(TAlarmI2T *p)
{
	...
	{
		if (*p->inputCurr > p->minHighCurrent )	// если ток превысил величину, при которой время-токовая защита срабатывает моментально
		{	
			p->isFault = true;			// выставляем аварию немедленно
			p->timer = 0;
		}
		else
