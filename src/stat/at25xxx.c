...
void AT25XXX_Init(AT25XXX *p)
{
	...
	if (AT25XXX_ReadStatus(p))
		p->Error = TRUE;				// Статусный регистр должен быть пустой
	asm(" NOP");
}
...
//-----------------------------------------------------------------
		case 2:																// Проверка статуса
			...
			if (!(p->Func & F_WRITE))
			{
				p->RdData = Tmp;
				p->Func = 0;
			}		// Отсутствует функция записи, записываем байт статуса в RdData, обнуляем функции
			else if (!(Tmp & 0x01))
				p->Func &= ~F_GETSTATUS;				// Проверяем бит RDY в статусном регистре, если отсутствует, то устройство готово для записи, отсекаем функцию чтения статуса
			else if (++p->BusyTimer < p->BusyTime)
			{
				p->State = 1;
				break;
			}	// Если функция записи и бит RDY выставлен (устройство еще занято), то проверяем таймер занятости устройства
			else
			{
				p->Error = TRUE;
				p->Func = 0;
...
//-----------------------------------------------------------------
		case 3:															// Чтение байта
			...
			{
				if (++p->RetryTimer < p->RetryCount)
					p->Func &= ~F_READ;	// Инкрементируем счетчик попыток и проверяем, если ли еще попытки. Если есть, то отсекаем функцию чтения,
										// чтобы потом по статусу проверить готовность устройства и повторить цикл записи
				else
				{
					p->Error = TRUE;
					p->Func = 0;
				}						// Если попытки на перезапись кончились, то выставляем ошибку
				p->State = 0;
