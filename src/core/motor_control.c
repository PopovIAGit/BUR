...
void TorqueCalc(void)	// расчет момента по кубу
{
...
#if !TORQ_TEST
	if(Dmc.RequestDir > 0)	// В открытие
	{
		if (Dmc.TorqueSetPr < 30)		  Add = GrC->Corr25TrqOpen;
		else if (Dmc.TorqueSetPr < 40) 	  Add = GrC->Corr40TrqOpen;		// Добавил PIA 09.10.2012
		else if (Dmc.TorqueSetPr < 60)	  Add = GrC->Corr60TrqOpen;		// корректировка индикации момента для +- изменения времени перехода на поверхность упора
	...
	else	// В закрытие
	{
		...
		else if (Dmc.TorqueSetPr < 40) 	  Add = GrC->Corr40TrqClose;		// Добавил PIA 09.10.2012
		else if (Dmc.TorqueSetPr < 60)	  Add = GrC->Corr60TrqClose;		// корректировка индикации момента для +- изменения времени перехода на поверхность упора
...
//-----------------Сарапульские------------------------------------------------------
		case dt100_A25_S:   PFUNC_blkRead(&drive11,   			(Int *)(&Ram.GroupH.TqCurr), LENGTH_TRQ);
				...
				case dt10000_D12_S: PFUNC_blkRead(&drive20,  			(Int *)(&Ram.GroupH.TqCurr), LENGTH_TRQ);
					...
					break; //22
				    case dt10000_D12_U2:
					PFUNC_blkRead(&drive24, (Int *) (&Ram.GroupH.TqCurr), LENGTH_TRQ);
					GrH->UporOnly = GrC->UporOnly;
					PFUNC_blkRead(&TransCurrDef[10], (Int *) (&Ram.GroupH.TransCurr), 1);
